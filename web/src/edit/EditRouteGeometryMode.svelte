<script lang="ts">
  ...
  // These are readonly, only relevant for existing routes
  let name = "";
  let notes = "";
  ...
  async function finish() {
    try {
      ...
      let newIDs = await $backend!.setRoute(id, {
        feature,
      ...
      await autosave();
      $mode = { kind: "edit-attributes", ids: newIDs };
    } catch (err) {
      window.alert(err);
    }
  }
...
<RouteControls {map} {finish} {cancel} editingExisting={id != null}>
